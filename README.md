...
- ``.env``: Environment configuration. It contains the following IMPORTANT variables:  
    - ``PORT``: used 8889, but the default is 8888 for Jupyter;  
    ...
    - ``CENSYS``: the path where the censys data is located. Pay attention to the directory structure (under censys longitudinal analysis of ``analysis.ipynb`` notebook.)
...
We used Spark version 3.5.0, with Python version 3.11.6 (both versions extracted from the running Jupyter notebook itself). Every needed python module was installed directly via the Jupyter notebook. The following Jupyter configuration was used:  
```
!jupyter --version
